...
async function bulkUpdateLeadsDateBased() {
  try {
    console.log('Starting date-based bulk update from logs...');
    
    // Get all emails from logs
    const emails = logData.map(d => d.email.toLowerCase());
    
    // Get leads from database with their creation dates
    const leadsResult = await pool.query(
      'SELECT id, email, created_at, keyword_id, campaign_id, ad_group_id, gclid FROM leads WHERE email = ANY($1)',
      [emails]
    );
    ...
    // Build a map of email -> log data
    const logMap = {};
    logData.forEach(log => {
      logMap[log.email.toLowerCase()] = log;
    });
    ...
    for (const lead of leadsResult.rows) {
      ...
      // Look up the keyword in daily_stats for that specific date
      const statsResult = await pool.query(
        `SELECT ds.keyword_id, ds.campaign_id, ds.ad_group_id, k.text as keyword_text
         ...
         LIMIT 1`,
        [leadDate, log.keyword]
      );
      
      if (statsResult.rows.length === 0) {
        console.log(`No stats found for ${log.keyword} on ${leadDate} for lead ${lead.email}`);
        noStatsFound.push({ email: lead.email, keyword: log.keyword, date: leadDate });
        continue;
      }
    ...
    if (noStatsFound.length > 0) {
      console.log('No stats found details:');
      noStatsFound.forEach(item => {
        console.log(`  ${item.email} - ${item.keyword} on ${item.date}`);
      });
    }
    ...
    for (const update of updates) {
      ...
      console.log(`Updated lead ${update.email} (${update.leadId}) - ${update.keywordText} on ${update.date}`);
    }
