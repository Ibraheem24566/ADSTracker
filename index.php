...
if (isset($_GET['crid']) && !empty($_GET['crid'])) {
    $crid = $_GET['crid'];
    if (file_exists('Google_CriteriaID_US.csv') && ($handle = fopen('Google_CriteriaID_US.csv', 'r')) !== false) {
        fgetcsv($handle); // skip header
        while (($row = fgetcsv($handle)) !== false) {
            if ($row[0] === $crid) {
                $city = htmlspecialchars($row[1], ENT_QUOTES, 'UTF-8');
                break;
            }
        }
        fclose($handle);
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* --- HERO SECTION --- */
        .hero {
            background: linear-gradient(rgba(0, 50, 80, 0.85), rgba(0, 84, 120, 0.9)), url('https://images.unsplash.com/photo-1509391366360-2e959784a276?ixlib=rb-1.2.1&auto=format&fit=crop&w=1920&q=80');
        ...
        @media(max-width: 900px) {
            ...
            .hero-proof-bar { margin: 0 auto 25px; }
            .usp-list { display: inline-block; text-align: left; }
        ...
        /* --- REVIEWS CAROUSEL --- */
        .carousel-section { padding: 80px 0; background: #f4f7f9; overflow: hidden; border-top: 1px solid #ddd; }
...
<body>
    ...
    <section class="hero">
        <div class="container hero-grid">
            <div class="hero-content">
                ...
                <p class="subhead">Join thousands of homeowners saving on energy bills with Venture Solar. $0 Down options available.</p>
                <ul class="usp-list">
    ...
    <section class="carousel-section">
        <div class="container text-center" style="margin-bottom:40px;">
            ...
            <p style="color:#666;">Hundreds of Real 5-Star Reviews from Google, BBB, and SolarReviews</p>
        </div>
        <div id="reviews" class="carousel-container">
    ...
    <script>
        ...
        (function() {
            ...
            document.getElementById('adset_id').value = params.get('adsetid') || '';
            // Add gclid field if it exists in the form
            const gclidField = document.getElementById('gclid');
            if (gclidField) {
                const gclidValue = params.get('gclid') || params.get('gclsrc') || '';
                gclidField.value = gclidValue;
                console.log('GCLID value:', gclidValue);
            }
        })();
